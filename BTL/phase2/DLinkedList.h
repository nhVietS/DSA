...
T DLinkedList<T>::removeAt(int index)
{
    ...
    while (cursor < index - 1)
    { //cursor largest = 2; index = 3
        pPrev = pPrev->next;
        cursor += 1;
    }
    //here: pPrev points to previous item (at index - 1)
...
bool DLinkedList<T>::removeItem(T item, void (*removeItemData)(T))
{
    ...
    while (pNode != tail)
    {
        ...
        if (found)
        {
            ...
            // if(pNode->next == tail) tail->prev = pRev; //update tail
            // if(pNode->prev == head) head->next = pRev->next; //update head
            // pNode->next = 0;
            // pRev->next->prev = pNode->prev;
            // pNode->prev =0;
